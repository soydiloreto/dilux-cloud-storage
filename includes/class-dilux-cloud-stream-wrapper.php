...
class CloudStreamWrapper {
    ...
    public static function activate_offloading() {
        ...
        if ($state === PluginState::OFFLOADING_ACTIVE) {
            ...
            remove_filter('upload_dir', [__CLASS__, 'filter_upload_dir'], 10);

            add_filter('upload_dir', [__CLASS__, 'filter_upload_dir'], 10, 1);
    ...
    private static function setup_admin_hooks() {
        ...
        add_action('load-update.php', [__CLASS__, 'tear_down']);           // General updates page
        add_action('load-update-core.php', [__CLASS__, 'tear_down']);      // WordPress core updates
        add_action('load-plugin-install.php', [__CLASS__, 'tear_down']);   // Plugin installation
        add_action('load-theme-install.php', [__CLASS__, 'tear_down']);    // Theme installation
    }
    ...
    public static function register_force_https_filters(): void {
        self::unregister_force_https_filters();
        add_filter('wp_get_attachment_url',     [__CLASS__, 'force_https_on_url'],    99, 1);
        add_filter('wp_calculate_image_srcset', [__CLASS__, 'force_https_on_srcset'], 99, 1);
        add_filter('style_loader_src',          [__CLASS__, 'force_https_on_url'],    99, 1);
        add_filter('script_loader_src',         [__CLASS__, 'force_https_on_url'],    99, 1);
    }
    ...
    public static function unregister_force_https_filters(): void {
        remove_filter('wp_get_attachment_url',     [__CLASS__, 'force_https_on_url'],    99);
        remove_filter('wp_calculate_image_srcset', [__CLASS__, 'force_https_on_srcset'], 99);
        remove_filter('style_loader_src',          [__CLASS__, 'force_https_on_url'],    99);
        remove_filter('script_loader_src',         [__CLASS__, 'force_https_on_url'],    99);
    }
    ...
    public function stream_open($path, $mode, $options, &$opened_path) {
        ...
        if (strpos($mode, 'r') !== false) {
            ...
            return false;
        } else if (strpos($mode, 'w') !== false || strpos($mode, 'a') !== false) {
    ...
    public function stream_flush() {
        ...
        if (($health['consecutive_failures'] ?? 0) >= 3) {
            ...
            if ($written !== false) {
                ...
                $fallbacks[] = ['path' => $this->path, 'time' => time()];
        ...
        try {
            $result = $cloud_client->upload_file($temp_file, $this->path, ['mime_type_from_path' => $this->path]);
        } catch (\Exception $e) {
        ...
        self::$stat_cache[$this->path] = [
            0 => 0,         'dev' => 0,
            1 => 0,         'ino' => 0,
            2 => 33188,     'mode' => 33188,  // Regular file with 0644 permissions
            3 => 1,         'nlink' => 1,
            4 => 0,         'uid' => 0,
            5 => 0,         'gid' => 0,
            6 => 0,         'rdev' => 0,
            7 => strlen($this->content), 'size' => strlen($this->content),
            8 => time(),    'atime' => time(),
            9 => time(),    'mtime' => time(),
            10 => time(),   'ctime' => time(),
            11 => -1,       'blksize' => -1,
    ...
    public function rename($path_from, $path_to) {
        ...
        try {
            ...
            $delete_result = ['success' => false, 'error' => $e->getMessage()];
        }
