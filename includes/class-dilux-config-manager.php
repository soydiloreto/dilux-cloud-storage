...
class ConfigManager {
    ...
    public static function save_provider_config($provider_config) {
        ...
        if (!empty($provider_array['cloud_provider'])) {
            ...
            if (!self::validate_provider_config($provider_array['cloud_provider'], $provider_array['provider_config'] ?? [])) {
    ...
    public static function get_failed_files() {
        return get_option(self::FAILED_FILES_OPTION, []);
    }
    ...
    public static function migrate_from_legacy() {
        $legacy_config = get_option('dilux_cs_config', []);
    ...
    private static function validate_config($config) {
        ...
        if (!empty($config['cloud_provider'])) {
            ...
            if (!self::validate_provider_config($config['cloud_provider'], $config['provider_config'] ?? [])) {
                return [
                    'valid' => false,
