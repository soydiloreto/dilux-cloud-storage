...
class DiluxOneCloudProvider implements CloudStorageClientInterface {
	...
	private function get_or_refresh_sas_token(): string {
		...
		$response = wp_remote_post($this->api_base_url . '/storage/sas-token', [
			'headers' => [
	...
	public function test_connection(): array {
		try {
			$response = wp_remote_get($this->api_base_url . '/auth/verify', [
				'headers' => [
					'Authorization' => 'Bearer ' . $this->api_key,
					'X-WP-Domain' => $this->wp_domain,
				],
	...
	public function get_stats(bool $force_refresh = false): array {
		...
		try {
			$response = wp_remote_get($this->api_base_url . '/stats', [
				'headers' => [
					'Authorization' => 'Bearer ' . $this->api_key,
					'X-WP-Domain' => $this->wp_domain,
				],
	...
	public function upload_file(string $local_path, string $remote_path, array $options = []): array {
		return $this->execute_with_retry(function () use ($local_path, $remote_path, $options) {
			try {
				if (!file_exists($local_path)) {
					return ['success' => false, 'url' => '', 'error' => 'Local file not found: ' . $local_path];
				}
				...
				if (!$fh) {
					return ['success' => false, 'url' => '', 'error' => 'Failed to open file for reading'];
				}
				...
				return ['success' => false, 'url' => '', 'error' => $error_msg];
	...
	public function download_file(string $remote_path, string $local_path): array {
		return $this->execute_with_retry(function () use ($remote_path, $local_path) {
			try {
				$remote_path = ltrim($remote_path, '/');
				$url = $this->build_azure_url($remote_path);
	...
	public function file_exists(string $remote_path): bool {
		try {
			return $this->execute_with_retry(function () use ($remote_path) {
				$remote_path = ltrim($remote_path, '/');
				$url = $this->build_azure_url($remote_path);
	...
	public function copy_blob(string $source_path, string $dest_path): array {
		return $this->execute_with_retry(function () use ($source_path, $dest_path) {
			try {
		...
		for ($attempt = 1; $attempt <= $max_retries; $attempt++) {
			try {
				return $this->execute_with_retry(function () use ($remote_path) {
					...
					return array_map(function (FileInfo $fi) {
						return $fi->toArray();
					}, $files);
				});
	...
	public function prepare_batch_upload_handle(array $file_info): array {
		...
		if (!file_exists($local_path)) {
			return ['success' => false, 'error' => 'File not found: ' . $local_path, 'file_handle' => null];
		}
		...
		try {
			...
			if (!$file_handle) {
				return ['success' => false, 'error' => 'Failed to open file for reading', 'file_handle' => null];
			}
			...
			return ['success' => false, 'error' => 'Exception: ' . $e->getMessage(), 'file_handle' => null];
		}
	}
	...
	public function prepare_chunked_upload_handle(array $file_info): array {
		...
		try {
			...
			if (!$fp) {
				return ['success' => false, 'error' => 'Failed to open file for chunked upload', 'file_handle' => null];
			}
			...
			while (!feof($fp)) {
				...
				if ($http_code !== 201) {
					...
					return ['success' => false, 'error' => $error_msg, 'file_handle' => null];
				}
			...
			return ['success' => false, 'error' => 'Chunked upload exception: ' . $e->getMessage(), 'file_handle' => null];
		}
	}
	...
	public function prepare_download_handle(array $file_info): array {
		...
		try {
			...
			if (!$file_handle) {
				return ['success' => false, 'error' => 'Failed to open local file for writing', 'file_handle' => null];
			}
			...
			return ['success' => false, 'error' => 'Download preparation exception: ' . $e->getMessage(), 'file_handle' => null];
		}
	}
	...
	private function is_non_retryable_error(string $error_code): bool {
		return in_array($error_code, ['400', '401', '403', '404', '409'], true);
	}
}
