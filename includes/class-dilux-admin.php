...
class Admin {
    ...
    public static function init() {
        ...
        \add_action('admin_post_dilux_cs_save_config', [__CLASS__, 'save_config']);
        \add_action('admin_post_dilux_cs_save_offloading', [__CLASS__, 'save_offloading_config']);
        \add_action('admin_post_dilux_cs_remove_provider', [__CLASS__, 'remove_provider_config']);
        
        // Register AJAX handlers
        \add_action('wp_ajax_dilux_cs_test_connection', [__CLASS__, 'ajax_test_connection']);
        \add_action('wp_ajax_dilux_cs_save_updated_credentials', [__CLASS__, 'ajax_save_updated_credentials']);
        \add_action('wp_ajax_dilux_cs_migration_action', [__CLASS__, 'ajax_migration_action']);
        \add_action('wp_ajax_dilux_cs_test_check', [__CLASS__, 'ajax_test_check']);
        \add_action('wp_ajax_dilux_cs_scan_files', [__CLASS__, 'ajax_scan_files']);
        ...
        \add_action('wp_ajax_dilux_cs_cancel_sync', [__CLASS__, 'ajax_cancel_sync']);
        \add_action('wp_ajax_dilux_cs_mark_sync_complete', [__CLASS__, 'ajax_mark_sync_complete']);
        \add_action('wp_ajax_dilux_cs_delete_local_files', [__CLASS__, 'ajax_delete_local_files']);
        \add_action('wp_ajax_dilux_cs_retry_failed', [__CLASS__, 'ajax_retry_failed']);
        \add_action('wp_ajax_dilux_cs_clear_failed', [__CLASS__, 'ajax_clear_failed']);
        \add_action('wp_ajax_dilux_cs_resync_all', [__CLASS__, 'ajax_resync_all']);
        \add_action('wp_ajax_dilux_cs_ajax_remove_provider', [__CLASS__, 'ajax_remove_provider']);
        \add_action('wp_ajax_dilux_cs_import_config', [__CLASS__, 'ajax_import_config']);
        \add_action('wp_ajax_dilux_cs_refresh_stats', [__CLASS__, 'ajax_refresh_stats']);
    ...
    public static function enqueue_admin_assets($hook_suffix) {
        ...
        wp_localize_script('dilux-cs-admin', 'diluxCloudStorageAdmin', [
    ...
    private static function render_tab_content($current_tab) {
        ...
        switch ($current_tab) {
            case 'overview':
                ...
                $is_configured_ov = !in_array($current_state_ov, ['not_configured', '']);
                $cloud_stats_ov = null;
                ...
                if ($is_configured_ov) {
                    try {
                        ...
                        $cloud_stats_ov = ['success' => false, 'message' => $e->getMessage()];
                    }
                }
            ...
            case 'cloud-provider':
                ...
                $is_configured_cp = !in_array($current_state_cp, ['not_configured', '']);
                $cloud_stats_cp = null;
                ...
                if ($is_configured_cp) {
                    ...
                    try {
                        ...
                        $cloud_stats_cp = ['success' => false, 'message' => $e->getMessage()];
                    }
            ...
            case 'activity':
                ...
                $template_data = [
                    ...
                    'chart_data' => ['labels' => [], 'uploads' => [], 'deletions' => []]
                ];
                break;
    ...
    public static function ajax_test_connection() {
        ...
        if (!$nonce_verified) {
            ...
            wp_send_json_error(['message' => esc_html__('Security check failed', 'dilux-cloud-storage')]);
            return;
        }
        
        // Check user permissions
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => esc_html__('Insufficient permissions', 'dilux-cloud-storage')]);
            return;
        }
        ...
        try {
            if ($provider === 'diluxone') {
                ...
                if (empty($api_key)) {
                    wp_send_json_error(['message' => esc_html__('API Key is required', 'dilux-cloud-storage')]);
                    return;
                }
                $client = \DiluxWP\CloudStorage\Factories\CloudStorageFactory::create('diluxone', [
                    'api_key' => $api_key,
                ]);
            } else {
                ...
                if (empty($account_name) || empty($account_key) || empty($container_name)) {
                    wp_send_json_error(['message' => esc_html__('Missing required fields', 'dilux-cloud-storage')]);
                    return;
                }
                $client = \DiluxWP\CloudStorage\Factories\CloudStorageFactory::create('azure', [
    ...
    public static function ajax_refresh_stats() {
        ...
        $client = ConfigManager::get_cloud_client();
        if (!$client) {
            wp_send_json_error(['message' => esc_html__('Cloud client not available', 'dilux-cloud-storage')]);
            return;
        }
        ...
        try {
            ...
            if ($client instanceof \DiluxWP\CloudStorage\Providers\DiluxOneCloudProvider) {
                ...
                wp_send_json_error(['message' => esc_html__('Unknown provider type', 'dilux-cloud-storage')]);
                return;
            }
            ...
            if ($stats['success']) {
                ...
                wp_send_json_error(['message' => $stats['message'] ?? 'Failed to fetch stats']);
            }
        } catch (\Exception $e) {
    ...
    public static function ajax_save_updated_credentials() {
        ...
        // Check user permissions
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => esc_html__('Unauthorized', 'dilux-cloud-storage')]);
            return;
        }
        ...
        if (!$test_data) {
            wp_send_json_error(['message' => esc_html__('You must test the connection first', 'dilux-cloud-storage')]);
            return;
        }
        ...
        if ($provider === 'diluxone') {
            ...
            if (($test_data['provider'] ?? '') !== 'diluxone' ||
                ...
                wp_send_json_error(['message' => esc_html__('Credentials do not match tested values. Please test again.', 'dilux-cloud-storage')]);
                return;
            }
    ...
    public static function ajax_migration_action() {
        ...
        if (empty($action)) {
            wp_send_json_error(['message' => esc_html__('No action specified', 'dilux-cloud-storage')]);
            return;
        }
    ...
    public static function ajax_test_check() {
        ...
        if (empty($check_type)) {
            wp_send_json_error(['message' => esc_html__('No check type specified', 'dilux-cloud-storage')]);
            return;
        }
        ...
        try {
            switch ($check_type) {
                ...
                default:
                    ...
                    wp_send_json_error(['message' => sprintf(esc_html__('Invalid check type: %s', 'dilux-cloud-storage'), $check_type)]);
                    return;
            }
    ...
    public static function ajax_import_config() {
        ...
        // Check permissions
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Insufficient permissions', 'dilux-cloud-storage')]);
            return;
        }
